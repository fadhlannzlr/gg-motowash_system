...
// =====================================================================
const Toast = Swal.mixin({ toast: true, position: 'top-end', showConfirmButton: false, timer: 2500, timerProgressBar: true, background: '#1e293b', color: '#f8fafc' });
...
function initChart() {
    ...
    dailyChart = new Chart(ctx, { type: 'bar', data: { labels: chartData.labels, datasets: [{ label: 'Pemasukan', data: chartData.pemasukan, backgroundColor: '#0ea5e9', borderRadius: 6 }, { label: 'Pengeluaran', data: chartData.pengeluaran, backgroundColor: '#ef4444', borderRadius: 6 }] }, options: { responsive: true, maintainAspectRatio: false, scales: { y: { beginAtZero: true, grid: { color: 'rgba(255,255,255,0.05)', borderDash: [4, 4] } }, x: { grid: { display: false } } }, plugins: { legend: { position: 'top' } } } });
}
...
function updateHistoryTableUI() {
    ...
    if (riwayatTransaksi.length === 0) {
        tbody.innerHTML = `<tr><td colspan="7"><div class="empty-state-wrapper"><i class="fa-solid fa-box-open"></i><p>Belum ada data transaksi yang tercatat.</p></div></td></tr>`;
        return;
    }
    ...
    riwayatTransaksi.forEach((item, index) => {
        ...
        tr.innerHTML = `
            ...
            <td><span style="text-transform: uppercase; font-size:0.8rem; font-weight:700; padding:4px 10px; background:rgba(14,165,233,0.15); color:#0ea5e9; border-radius:99px;">${item.metode}</span></td>
...
if (formPengeluaran) {
    formPengeluaran.addEventListener('submit', async (e) => {
        ...
        if (nominal > saldoSaatIni) return Swal.fire({ icon: 'error', title: 'Saldo Tidak Cukup!', text: `Kas tersisa ${formatRupiah(saldoSaatIni)}.`, background: '#1e293b', color: '#f8fafc' });
...
if (formStok) {
    formStok.addEventListener('submit', async (e) => {
        ...
        if (aksi === 'tambah') { stok[item] += jumlah; } 
            ...
            if (jumlah > stok[item]) return Swal.fire({ icon: 'error', title: 'Stok Kurang!', text: `Sisa ${stok[item]}.`, background: '#1e293b', color: '#f8fafc' });
            stok[item] -= jumlah;
        }
...
window.hapusBarisTransaksi = async function(id) {
    Swal.fire({ title: 'Hapus Transaksi?', icon: 'warning', showCancelButton: true, confirmButtonColor: '#ef4444', background: '#1e293b', color: '#f8fafc' }).then(async (result) => {
...
if (btnClearAll) {
    btnClearAll.addEventListener('click', async () => {
        ...
        Swal.fire({ title: 'Kosongkan Data?', icon: 'warning', showCancelButton: true, confirmButtonColor: '#dc2626', background: '#1e293b', color: '#f8fafc' }).then(async (result) => {
            if (result.isConfirmed) {
                Swal.showLoading();
                ...
                riwayatTransaksi = [];
                Swal.close(); updateDashboardUI(); Swal.fire({title: 'Terhapus!', text: 'Semua riwayat bersih.', icon: 'success', background: '#1e293b', color: '#f8fafc'});
            }
        });
    });
}
...
// =====================================================================
// 8. FITUR EKSPOR, CETAK STRUK, PENCARIAN & NOTIFIKASI
// =====================================================================
window.cetakStruk = function(id) {
    const t = riwayatTransaksi.find(x => x.id === id);
    if (!t) return;
    const printWindow = window.open('', '_blank');
    printWindow.document.write(`<html><head><title>Struk GG Steam</title></head><body style="font-family: monospace; width: 200px;"><div style="text-align: center;"><h3>GG MOTOWASH</h3><p>${t.waktu}</p><hr></div><p>Jasa: ${t.layananTeks}</p><p>Cust: ${t.pelanggan}</p><hr><p><b>TOTAL: ${formatRupiah(t.harga)}</b></p><p>Metode: ${t.metode.toUpperCase()}</p><div style="text-align: center;"><p>Terima Kasih!</p></div></body></html>`);
...
if (searchInput) {
    searchInput.addEventListener('input', function(e) {
        ...
        tbody.innerHTML = '';
        if (fData.length === 0) return tbody.innerHTML = `<tr><td colspan="7"><div class="empty-state-wrapper"><i class="fa-solid fa-search"></i><p>Pencarian "${keyword}" tidak ditemukan.</p></div></td></tr>`;
        
        fData.forEach((item, index) => {
            const tr = document.createElement('tr');
            tr.innerHTML = `<td>${index + 1}</td><td>${item.waktu}</td><td><strong>${item.pelanggan}</strong></td><td><span class="service-tag">${item.layananTeks}</span></td><td class="text-green" style="font-weight:700;">${formatRupiah(item.harga)}</td><td><span style="text-transform: uppercase; font-size:0.8rem; font-weight:700; padding:4px 10px; background:rgba(14,165,233,0.15); color:#0ea5e9; border-radius:99px;">${item.metode}</span></td><td style="text-align: center;"><button class="btn-delete-row" onclick="cetakStruk('${item.id}')" title="Cetak"><i class="fa-solid fa-print"></i></button><button class="btn-delete-row" onclick="hapusBarisTransaksi('${item.id}')" title="Hapus"><i class="fa-solid fa-trash-can"></i></button></td>`;
            tbody.appendChild(tr);
        });
...
if (btnNotif) {
    btnNotif.addEventListener('click', () => {
        ...
        let pesan = '<div style="text-align:left; font-size:0.95rem;">', ada = false;
        if (stok.sabun < 1000) { pesan += '<p style="color:#ef4444; margin-bottom:12px; padding:10px; background:rgba(239,68,68,0.1); border-radius:8px;"><i class="fa-solid fa-triangle-exclamation"></i> <b>Stok Sabun Kritis:</b> Kurang dari 1000 ml!</p>'; ada = true; }
        if (stok.semir < 500) { pesan += '<p style="color:#f59e0b; margin-bottom:12px; padding:10px; background:rgba(245,158,11,0.1); border-radius:8px;"><i class="fa-solid fa-circle-exclamation"></i> <b>Stok Semir Menipis:</b> Kurang dari 500 ml!</p>'; ada = true; }
        if (!ada) pesan += '<p style="color:#10b981; padding:10px; background:rgba(16,185,129,0.1); border-radius:8px;"><i class="fa-solid fa-circle-check"></i> <b>Sistem Aman:</b> Metrik keuangan dan stok stabil.</p>';
        Swal.fire({ title: 'Pusat Notifikasi', html: pesan + '</div>', icon: ada ? 'warning' : 'info', confirmButtonColor: '#0ea5e9', confirmButtonText: 'Tutup', background: '#1e293b', color: '#f8fafc' });
    });
}
...
if (btnExportExcel) {
    btnExportExcel.addEventListener('click', () => {
        ...
        const ws = XLSX.utils.json_to_sheet(dataExcel), wb = XLSX.utils.book_new(); XLSX.utils.book_append_sheet(wb, ws, "Laporan"); XLSX.writeFile(wb, `Laporan_GG_Motowash_${new Date().toISOString().slice(0,10)}.xlsx`);
    });
}
...
if (btnExportPDF) {
    btnExportPDF.addEventListener('click', () => {
        ...
        doc.setFontSize(11); doc.setFont("Helvetica", "normal"); doc.text(`GG Motowash | Tanggal: ${new Date().toLocaleDateString('id-ID')}`, 14, 28);
        ...
        doc.save(`Laporan_GG_Motowash_${new Date().toISOString().slice(0,10)}.pdf`);
    });
}
...
if (btnLogout) {
    btnLogout.addEventListener('click', () => {
        Swal.fire({ title: 'Akhiri Sesi?', icon: 'question', showCancelButton: true, confirmButtonColor: '#0ea5e9', confirmButtonText: 'Keluar', background: '#1e293b', color: '#f8fafc' }).then(async (r) => { 
            if (r.isConfirmed) { await auth.signOut(); window.location.href = 'index.html'; }
        });
    });
}
